...
class TwitterController():
    ...
    def get_following_list(self):
        following_list = []
        for friend in tweepy.Cursor(self.twitter_controller.friends,id=self.twitter_user).items():
            following_list.append(friend)
        return following_list
    def get_selected_following(self,user):
        following_list = []
        for friend in tweepy.Cursor(self.twitter_controller.friends, id=user).items():
            following_list.append(friend)
        return following_list
    def get_followers(self):
        follower_list = []
        for follower in tweepy.Cursor(self.twitter_controller.followers, id=self.twitter_user).items():
            follower_list.append(follower)
        return follower_list
    def get_selected_followers(self,user):
        follower_list = []
        for follower in tweepy.Cursor(self.twitter_controller.followers, id=user).items():
            follower_list.append(follower)
        return follower_list
